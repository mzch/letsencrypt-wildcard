...
if [[ "$VD_NAMESERVER" != "valuedomain1" && "$VD_NAMESERVER" != "valuedomain11" ]] ; then
...
eval ${CURL} | jq -r '.results.records' | awk '{ if (NF > 2) { print $0; }}' > "$TMPFILE"
...
DNSREQ=${DNSREQ}"\\ntxt _acme-challenge \\\"${CERTBOT_VALIDATION}\\\"\",\"ttl\":\"3600\"}"
