...
```bash
wild-vd.sh example.com [メールアドレス] [コマンド実行間隔]
```
...
第3引数の「コマンド実行間隔」は、Value-Domain のネームサーバーへの変更が反映されるのに充分な秒数を指定します。デフォルトは 10 秒です。環境変数 `LE_SLEEP_SECS` でデフォルトを変更することができます。
...
|`LE_SLEEP_SECS`| コマンド実行間隔のデフォルト値を指定します。無指定の場合、10 がデフォルトとなります|
